  ...
  var QueueManager = {
    ...
    _runModel: function (item, res) {
      ...
      if (item.mode === "upscale") {
        window.ModelHandler.upscaleClip(inputPath, outputPath, item.model, { scale: String(item.scale), targetSizeMb: item.targetSizeMb || 0, preset: item.preset || "slower" }, callbacks);
      } else if (item.mode === "dedupe") {
        window.ModelHandler.dedupeClip(inputPath, outputPath, item.threshold || 0.05, item.options || {}, callbacks);
      } else {
        window.ModelHandler.interpolateClip(inputPath, outputPath, item.model, { fpsFactor: String(item.factor), targetSizeMb: item.targetSizeMb || 0, preset: item.preset || "slower" }, callbacks);
      }
    }
  };
